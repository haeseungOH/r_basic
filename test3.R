# DataFrame : 데이터베이스의 테이블과 유사한 구조. 열단위로 type이 다를 수 있다.
...
# subset(mdf,select = c(2)) = 열 선택 (2열만 선택)

mdf
mdf$c2 <- ifelse(mdf$c2==4,NA,mdf$c2) #if문 
mdf

summary(mdf)

mean(mdf$c2)
mean(mdf$c2,na.rm = T) # na를 제외하고 평균을 계산하기
...
weight <- data.frame(id=c(1,2,2),h=c(80,75,88))
height
weight
merge(height,weight, by.x = "id",by.y = "id") #id를 기준으로 병합
