...
weather_df <- na.omit(weather_df) # NA 삭제
sum(is.na(weather_df)) # NA 갯수 확인 

unique(weather_df$RainTomorrow)   # "Yes" "No"

# RainTommorrow 변수를 더미(dummy)변수화 
...
test <- weather_df[-idx,]
dim(train); dim(test)    # 256   9, 110   9 , model 의 과적합(overfitting) 방지가 목적
...
plot(prf)
# 커브 선이 왼쪽 상단에 가까울수록 좋은 model로 판정한다.
