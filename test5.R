...
library(ggplot2)
head(mpg,2) # utils::head(mpg,2) 라고 쓸수도 있다.
typeof(mpg)
is(mpg)
mpg <- as.data.frame(ggplot2:mpg) # mpg는 ggplot2 가 가지고 있는 library라고 알려주는 방법
head(mpg,2)
? mpg
summary(mpg)
str(mpg)       # mpg 구조 확인
...
summary(mpg$total)
hist(mpg$total)      #Histogram 그리기 , 중심극한정리 
...
# 조건판단문 which : vector 를 대상으로 특정 data를 검색
