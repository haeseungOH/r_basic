...
dim(states)      # 50  5
cor(states)      # 상관관계 확인
# 독립변수 : 'Population','Illiteracy','Income','Frost'  = data
# 종속변수 : 'Murder'                                    = 답 

# model 작성
mfit <- lm(formula = Murder ~ Population+Illiteracy+Income+Frost, data = states)
summary(mfit)   # p-value: 9.133e-08 < 0.05이므로 유의한 모델. Adjusted R-squared: 0.5285 의 설명력을 갖는다.

# 새값으로 예측
part_states <- states[1:3,]
pre_states <- edit(part_states)   # edit로 칼럼값 수정
pre_states
...
# 1) 잔차의 정규성 검정
shapiro.test(residuals(mfit))   # W = 0.98264, p-value = 0.6672 > 0.05 정규성 만족 
...
# 3) 선형성 검정  
boxTidwell(Murder ~ Population+Illiteracy, data = states)   # 2개씩 넣을 수 있다.
...
# AIC 통계량 : 통계 모델간의 성능 비교용 
AIC(mfit1,mfit2)
# df      AIC
# mfit1  6 241.6429
# mfit2  4 237.6565  AIC가 낮은 mfit2 model이 더 좋다라고 평가됨 
...
min_model <- lm(Murder ~ 1, data = states)   # 변수없이 출발할때 절편값만 적는다.
...
library(leaps)
model <- leaps::regsubsets(Murder ~ Population+Illiteracy+Income+Frost, data=states, nbest=4)
summary(model)
summary(model)$bic
summary(model)$adjr2
